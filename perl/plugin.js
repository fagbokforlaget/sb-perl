builder.selenium2.io.addLangFormatter({
  ...
  not: "! ",
  start:
  "not really sure how this should start :)",
  end:
    "$wd->close();",
  lineForType: {
    "get":
  ...
  assert: function(step, escapeValue, doSubs, getter) {
    if (step.negated) {
      return doSubs(
        // I'm not really sure how to compare in perl, I'm assuming it's like in other languages
        // then: throw an exception in perl, how?
        // I'm leaving php code for now
        "if ({getter} == {cmp}) {\n" +
        " $wd->close();\n" +
